...
export default function ResetPassword({ onNavigate }: { onNavigate: (route: string) => void }) {
  return (
      ...
      <div className="flex-1 bg-surface flex items-center justify-center p-6 md:p-12 xl:p-20 relative z-20 rounded-l-3xl lg:rounded-none">
        ...
        <div className="w-full max-w-sm">
          <div className="mb-8">
            ...
            <h1 className="font-display text-3xl font-bold text-navy mb-2 tracking-tight">Đặt lại mật khẩu</h1>
            <p className="text-text-muted text-sm leading-relaxed">Vui lòng thiết lập mật khẩu mới và tuân thủ các quy định bảo mật bên dưới.</p>
          </div>
          ...
          <form onSubmit={(e) => { e.preventDefault(); onNavigate('login'); }} className="space-y-5">
            <div className="space-y-2">
              ...
              <div className="flex gap-2 mt-2">
                 <div className="h-1.5 flex-1 bg-success rounded-full"></div>
                 <div className="h-1.5 flex-1 bg-success rounded-full"></div>
                 <div className="h-1.5 flex-1 bg-surface-highest rounded-full"></div>
              </div>
              <p className="text-xs text-text-muted font-medium pt-1">Bảo mật: Trung bình</p>
            </div>
            
            <div className="space-y-3 bg-white p-4 rounded-xl border border-surface-highest mt-6">
               <label className="flex items-center gap-2 text-xs font-semibold text-success"><span className="w-4 h-4 rounded-full bg-success/20 flex items-center justify-center text-[10px]">✓</span> Ít nhất 8 ký tự</label>
               <label className="flex items-center gap-2 text-xs font-semibold text-success"><span className="w-4 h-4 rounded-full bg-success/20 flex items-center justify-center text-[10px]">✓</span> Có chữ hoa và số</label>
               <label className="flex items-center gap-2 text-xs font-semibold text-text-muted"><span className="w-4 h-4 rounded-full bg-surface-highest flex items-center justify-center text-[10px] text-text-muted/50">✗</span> Có ký tự đặc biệt</label>
            </div>
            ...
            <Button type="button" onClick={() => onNavigate('login')} className="w-full mt-4 text-sm font-semibold text-text-muted hover:text-navy transition-colors text-center block">Quay lại đăng nhập</Button>
          </form>
        </div>
      </div>
    </div>
